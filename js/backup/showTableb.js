...
function makeTableHTML(content){
    //HTML TEGS
    ...
    var numOfLink = (function(){
                ...
                for(i=0;i<contents[0].values.length;i++){
                    ...
                    for(ii=0; ii < (contents[0].values[i].length);ii++)
                        if(ii==numOfLink)tableDYnSql+=td+makeLinkHTML(contents[0].values[i][ii])+tdC;
...
function showTableFromSQL (dbPath,SQLQuery,callback){
            //CONNECT TO DB
